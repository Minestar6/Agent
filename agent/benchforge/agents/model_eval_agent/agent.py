...
def _generate_evaluation_session_id() -> str:
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    suffix = secrets.token_hex(2)
    return f"evaluation_{ts}_{suffix}"


def _load_shared_state(path: str | Path) -> dict:
    with open(path, encoding="utf-8") as f:
        return json.load(f)


def _resolve_input_paths_from_shared_state(
    shared_state: dict, run_dir: Path
) -> list[str]:
    artifacts = shared_state.get("artifacts", {})
    paths = []

    if val := artifacts.get("validated_questions"):
        paths.append(val)
    if paths:
        return paths
...
async def _run(
    ...
    evaluation_session_id = run.evaluation_session_id or _generate_evaluation_session_id()
    output_root = Path("runs") / task_id / run_id / "evaluation" / evaluation_session_id
    llm_trace_path = str(output_root / "llm_calls.jsonl")

    logger.info(
        f"[ModelEvalAgent] start task={task_id} run={run_id} session={evaluation_session_id}"
    )
...
async def run_model_eval_agent_from_shared_state(
    ...
    """流水线模式：task_id / run_id / input_paths 从 shared_state.json 读取。"""
    ...
    return await _run(effective_config, registry_path=registry_path)

...
class TestE2EHappyPath:
    def test_run_verify_agent_produces_outputs(self, tmp_path):
        ...
        assert result.validation_session_id.startswith("validation_")
    ...
    def test_validation_session_id_auto_generated(self, tmp_path):
        """不传 validation_session_id 时自动生成。"""
        run_dir = tmp_path / "runs" / "task_e2e" / "run_e2e"
        run_dir.mkdir(parents=True)
        pool_file = _make_candidate_pool(run_dir, n=2)

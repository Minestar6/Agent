...
import json
import tqdm
from pipelines.chunk import split_into_token_chunks
from utils.model import Model, _get_encoder, extract_content_from_xml_tags
from settings import UTILS_DIR
...
def _generate(model,messages,outfile, bsz=1, temperature=0.01, max_length=50):
    ...
    for line in tqdm.tqdm(messages):
        ...
        for prompt,res in zip(batch_lst,request_result):
            ...
            print(json.dumps({'prompt':prompt,'response':res}), file=out_handle)
        batch_lst = []
...
def run() -> None:
    ...
    with open(UTILS_DIR / "document1.json", "w",encoding='utf8') as f:
